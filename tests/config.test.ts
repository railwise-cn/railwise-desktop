...
describe("config", () => {
  ...
  it.runIf(process.platform === "win32")(
    ...
    () => {
      addProjectShellAllowed("F:\\Railwise", "gh", path);
      expect(loadProjectShellAllowed("f:\\railwise", path)).toContain("gh");
      ...
      addProjectShellAllowed("f:\\railwise", "deploy", path);
      expect(loadProjectShellAllowed("F:\\Railwise", path)).toEqual(["gh", "deploy"]);
      expect(Object.keys(readConfig(path).projects ?? {})).toEqual(["F:\\Railwise"]);
      expect(removeProjectShellAllowed("f:\\REASONIX", "gh", path)).toBe(true);
      expect(loadProjectShellAllowed("F:\\Railwise", path)).toEqual(["deploy"]);
      expect(clearProjectShellAllowed("F:\\REASONIX", path)).toBe(1);
      expect(loadProjectShellAllowed("F:\\Railwise", path)).toEqual([]);
    },
  );
  ...
  it("round-trips semantic embedding config", () => {
    ...
    expect(loaded.openaiCompat?.extraBody).toEqual({ user: "railwise" });
  });
