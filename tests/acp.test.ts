...
import { resolveApprovalPrompt, toApprovalPrompt, toolKindFor } from "@railwise/core-utils";
...
describe("ACP initialize handshake (end-to-end via the server)", () => {
  it("implements initialize → returns protocolVersion + agentCapabilities + agentInfo", async () => {
    ...
    server.onRequest("initialize", () => ({
      ...
      agentInfo: { name: "railwise", title: "Railwise", version: "0.0.0-test" },
      authMethods: [],
    }));
    send({
      jsonrpc: "2.0",
    ...
    expect(reply.result.agentInfo.name).toBe("railwise");
