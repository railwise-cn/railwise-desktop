    ...
    case "rpc_exit":
      return {
        ...
        messages: [
          ...
          {
            kind: "error",
            message: `railwise exited (code ${action.code ?? "?"})`,
            id: nextErrorId(),
          },
        ],
      };
...
export function App() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("railwise:currency", onCur);
    return () => window.removeEventListener("railwise:currency", onCur);
  }, []);
    ...
    const setup = async () => {
      ...
      const subs = await Promise.all([
        ...
        listen<{ data: string }>("rpc:stderr", (e) => {
          ...
          console.warn("[railwise stderr]", e.payload.data);
        }),
        listen<{ code: number | null }>("rpc:exit", (e) => {
          ...
          if (dispatchersRef.current.size === 0) {
            setStartupFailure(
              coerceStartupFailure(
                new Error(`railwise exited (code ${e.payload.code ?? "?"})`),
                startupStderrRef.current,
              ),
            );
          }
  ...
  const onToggleCurrency = useCallback(() => {
    setCurrency((c) => {
      ...
      window.dispatchEvent(new CustomEvent("railwise:currency", { detail: next }));
      return next;
    });
  }, []);
