...
Thanks for showing up. Railwise is a small, opinionated codebase
...
- **New features / behavior changes** — open an issue first to align
  on scope and approach. Railwise tries to stay small; "we could add
...
- **External MCP servers, plugins, presets** — a thin wrapper is
  ...
  package that depends on `railwise`.
...
Railwise prefers blunt, fast review. Expect:
