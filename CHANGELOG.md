...
other local AI apps can be imported into Railwise (#1894).
...
user memory directly into Railwise without rewriting it.
...
railwise@0.50.0 code` failed during install with a `dashboard/package.json
...
`railwise` (#1440), so `npx dsnix` is now equivalent to `npx railwise`
...
decision Railwise made for each host (#1375). If proxy behavior ever
...
(#1387), giving Railwise structural code navigation instead of plain
...
**Hotfix — `npx railwise@latest` install.** `0.47.1` shipped with
`@railwise/core-utils` listed under runtime `dependencies` as
...
**Other:**
- `feat(core-utils)` new `@railwise/core-utils` workspace package —
...
**Claude-ecosystem compat.** Railwise now reads `.mcp.json` and
...
**Breaking — Rust renderer removed.** Railwise is back to a pure Ink/Node
TUI. The `reasonix-render` ratatui crate, the five
`@railwise/render-{platform}-{arch}` optional sub-packages, the NAPI
...
- `--no-alt-screen` / `--no-mouse` flags also gone — Ink defaults take
  over end-to-end.
- `npm install railwise@0.46.0` no longer pulls a per-platform binary —
...
**Fix:** macOS hang on `npx railwise@next code` — keep-alive interval
...
**Fix:** macOS `npx railwise code` (default rust + integrated TUI) exited
...
**Fix:** Mac/Linux `npx railwise@latest` failed with `EACCES` when spawning
...
- Each `@railwise/render-*` subpackage now declares its binary in the
...
**Headline:** The Rust TUI is now the default TUI. `npx railwise@latest`
...
Bare `railwise` (no subcommand) routes to `code` in the cwd instead of
chat — explicit `railwise chat` still works.
...
- feat(cli): bare `railwise` (no subcommand) now launches `code` in
  the cwd. Drops the project-marker heuristic that used to route to
  `chat` outside of project dirs; `railwise chat` stays explicit (#1081)
- feat(scene): renderer-resolver picks the binary by priority chain —
  ...
  `@railwise/render-{platform}-{arch}` optional dependency > source-
...
- feat(release): per-platform `@railwise/render-*` subpackages
...
- fix(code): honor the configured preset instead of hardcoding flash.
  (#824)
- fix(cli): route bare project launches (`railwise` with no
...
- fix(dashboard): swallow third-party-origin errors in the global
  overlay — extension scripts no longer surface as Railwise crashes.
  (#825)
...
`--mcp-prefix`) make `railwise acp` ready for non-interactive callers
...
- feat(acp): three-stage rollout of `railwise acp`, the headless ACP
...
client (`railwise desktop`) wired against the agent loop with multi-
...
window), `railwise code` no longer boots dead when a user has only
configured their key via `railwise setup`, and a half-dozen MCP /
...
- fix(tui): CardStream window position is now quantized to
  ...
  pattern). (#700, #702)
- fix(code): `railwise code` now calls `loadDotenv()` + bridges
...
when railwise is installed as a dependency. The install-time
...
- feat(ui): boot splash for `railwise code` / `railwise chat`. Cold
...
- fix(prompt): when the workspace root contained another agent
  ...
  what Railwise is; identity questions are answered from the prompt,
...
`railwise update` (no more forced `npm install -g` for bun/pnpm
...
- feat(update): `railwise update` respects the install source
...
- feat(tools): `create_skill` + `add_mcp_server` — let the model
  ...
  alongside native filesystem / shell tools in `railwise code`.
  (#498, closes #494)
...
- fix(doctor): tokenizer check now finds the file. The runtime
  ...
  `createRequire("railwise/package.json")` probe and worked
...
for the chat UI only when `railwise code` actually runs. `railwise
version` and `railwise --help` drop ~290ms (~440ms → ~140ms);
`railwise code` is unchanged on the hot path. Critical bug fix at
...
- perf(cli): lazy-import every per-command module. Each
  `railwise <subcommand>` only loads its own command's chunk. tsup
  splits, Node loads on first invocation. `railwise version` and
  `railwise --help` drop ~290ms (~440ms → ~140ms); `railwise code`
...
**Headline:** a Mac user reported a DeepSeek 503 day where Railwise
...
- feat(loop): friendly DeepSeek 5xx error with reachability probe.
  ...
  a DeepSeek-side problem, not Railwise" and link
...
- feat(wizard): first-launch language picker + full i18n. New
  ...
  language. Re-running `railwise setup` opens at the same step with
...
- fix(tty): drain pending feature-detection replies on exit. Linux
  ...
  fish / bash after exiting railwise — those bytes are responses to
...
- feat(ui): surface `--dir` / pinned workspace for first-time users.
  ...
  filesystem sandbox-escape error points at `railwise code --dir
...
18.3 → 19.2, the cell-diff renderer is retired, and `railwise code` /
`railwise chat` default to alt-screen with row-precision virtual
...
- feat(prompt): full readline shortcut set wired into the prompt
  ...
  `Ctrl+U` keeps Railwise's "clear whole buffer" behaviour, not
...
- feat(mcp): new `/mcp reconnect <name>` slash subcommand. Re-handshakes
  ...
  reorder / remove drift is refused with a clear "restart Railwise to
...
- `engines.node` bumped from `>=20.10` to `>=22`. Node 20 reached
  end-of-life on 2026-04-30; `npm install railwise` on Node 20 will now
  print an `EBADENGINE` warning. Tested CI surface trimmed to a single
  Node 22 job. (#98)

**Fixes:**

- fix(code): `railwise code` now bridges MCP servers from
  `~/.reasonix/config.json`, matching `railwise chat` behaviour.
...
- docs(readme): designer pass — drop redundant `# Railwise` H1 (the
...
**Headline:** `railwise index` is now config-driven — what gets walked
...
- feat(index): glob excludes via `picomatch` syntax in
  ...
  supported).
- feat(cli): `railwise index` success line now prints a per-reason
...
API"). Railwise's whitelist-by-model in `assistantMessage()` was too
...
on v0.16.0: 24/24 railwise runs were failing.
...
- feat(cli): `railwise events <name>` — inspect any session's event
  stream from the command line. Filters by event variant
  (`railwise events ToolCallStarted`), tail mode, JSON output for
...
Previously you had to exit the TUI, run `railwise index`, wait,
...
- Standalone `railwise dashboard` mode shows a polite "code-mode
...
- Auto-launch the embedded dashboard when `railwise code` /
  `railwise chat` mount. Failures are silent (a missing dashboard
...
- `cacheSavingsUsd(model, hitTokens)` in `src/telemetry.ts` — USD
  ...
  for cached tokens). Surfaced in `railwise stats` dashboard +
...
- **Spec parser** — `streamable+http(s)://` prefix routes to the
  ...
  Wired through `chat.tsx`, `run.ts`, and `railwise mcp inspect`.
...
backed embedding index, `railwise index` CLI with progress spinner, a
...
- **`railwise index`** — new CLI command that walks the project, line-
...
- **`semantic_search` tool** — registered in `railwise code` only when
...
- **Default model is now `deepseek-v4-flash`**, not `deepseek-v4-pro`.
  `railwise code`, `railwise chat`, and subagents all land on flash
...
**Headline:** `railwise code` gets a proper review gate, background
...
- **Onboarding tip** — first `railwise code` launch after upgrade
...
**Headline:** `railwise stats` is now a cross-session cost dashboard.

Every turn `railwise chat|code|run` executes now appends one line to
`~/.reasonix/usage.jsonl` carrying tokens + cost + the equivalent
Claude Sonnet 4.6 cost. `railwise stats` (no arg) rolls that log up
into today / week / month / all-time windows:

```
Railwise usage — /Users/you/.reasonix/usage.jsonl (2.3 KB)
...
Back-compat: `railwise stats <transcript>` still works — passing a
...
- **Wire-up** in `src/cli/ui/App.tsx` (assistant_final event) and
  ...
  log as TUI turns).
- **Upgraded `railwise stats`**. No-arg → dashboard; transcript arg
...
A hook is a shell command. Railwise invokes it with stdin = a JSON
...
- **`/update` slash command**. Shows current vs the last-resolved
  ...
  and run `railwise update` in a fresh shell.
...
**Headline:** Version display in the TUI header + `railwise update`
self-upgrade command.

Two small quality-of-life additions. The stats panel now carries the
running version (`Railwise v0.4.22 · model …`) so users can tell at
...
`railwise update` is the command form: detects whether you're
running a global install vs an ephemeral `npx` spawn, and either
spawns `npm install -g railwise@latest` for the former or prints a
...
- **`railwise update`** subcommand (`src/cli/commands/update.ts`).
...
Railwise discovers skills under `<project>/.reasonix/skills/` (project
...
convention (`.claude/`, `.glm/`, etc.): Railwise is model-agnostic at
...
- **`run_skill` tool** (`src/tools/skills.ts`) — read-only, returns
  ...
  Registered in `railwise chat` (global only) and `railwise code`
  (project + global).
...
- Each skill's `allowed-tools` frontmatter is parsed but **ignored**
  in v1. Railwise's tool namespace (`filesystem`, `shell`, `web`)
...
- **`ShellConfirm` "always allow" did not take effect until relaunch.**
  ...
  addition to a static array; `railwise code` passes
...
`railwise code` now runs `npm`, `npx`, `tsc`, `yarn`, `pnpm`, `bun`,
...
- **`submit_plan` tool** (`src/tools/plan.ts`) — registered by default
  in `railwise code`. Throws `PlanProposedError` carrying the plan
...
- **`src/project-memory.ts`** — `readProjectMemory(rootDir)`,
  ...
  library consumers.
- **Auto-applied at every CLI entry** — top-level `railwise`,
  `railwise chat`, `railwise run`, and `railwise code` all honor
...
"deny". Plus a session picker on startup so `railwise code` stops
silently resuming the last conversation, and a Windows backspace fix.

### Added

- **`src/tools/shell.ts`** — `run_command(command, timeoutSec?)`
  registered by default in `railwise code`. Read-only / testing
...
- **`src/cli/ui/SessionPicker.tsx`** — on `railwise chat` /
  `railwise code` startup, if the session has prior messages, show
...
- **Per-project persistent allowlist** — `config.projects[<abs>].shellAllowed`
  ...
  `railwise code` in that dir they auto-run. Helpers
...
Web search + fetch tools are registered by default on `railwise
chat` and `railwise code`. The model calls `web_search` /
...
- **`ReasonixConfig.search`** + **`searchEnabled()`** — a simple
  ...
  picker — one switch.
- **Auto-registered in chat/code.** `railwise chat` and
  `railwise code` register `web_search` + `web_fetch` by default.
...
- **`railwise --version` no longer reports 0.4.3 forever.** The
...
**Headline:** Two streaming-row bugs that made `railwise code` feel
...
really stops (not "after the tool finishes"), `railwise code` drops
...
- **Built-in filesystem tools** replace the
  ...
  `railwise code`. Ten tools — `read_file` (head/tail), `write_file`,
  ...
  path. New CLI output: `▸ railwise code: … · 10 native fs tool(s)`.
...
- **`slow_count` demo tool** in `examples/mcp-server-demo.ts` that
  ...
  end-to-end: `railwise chat --mcp "demo=node --import tsx examples/mcp-server-demo.ts"` then ask the model to
...
`railwise mcp inspect <spec>` CLI doing the same.
...
- **`railwise mcp inspect <spec>`**. CLI counterpart to `/mcp`, for
...
after live `railwise code` sessions surfaced pain points: R1 fake
...
**Headline:** `railwise code` grows `/undo`, `/commit`, `.gitignore`
...
- **.gitignore awareness** — `railwise code` reads the project's
...
**Headline:** `railwise code` — a new subcommand that turns Railwise
...
- **`npx railwise code [dir]`** — opinionated wrapper around chat:
...
- **SEARCH/REPLACE edit blocks** (`src/code/edit-blocks.ts`). The
  model emits:
    ...
    >>>>>>> REPLACE
    ```
  Railwise parses them from `assistant_final`, applies them under
...
- **`ChatOptions.codeMode`** — opt-in flag to enable edit-block
  processing inside the existing TUI event loop. Plain `railwise chat`
  leaves it off.
...
Nothing breaks for existing 0.3.x users — `railwise chat` behaves
...
- **README hero rewrite.** `npx railwise` (no flags) is now the first
...
series graduates — `npm install railwise@latest` now pulls this.

### Added — since 0.2.2

- **MCP client**: stdio + HTTP+SSE transports, tools/list + tools/call,
  repeatable `--mcp` flag with `name=` namespacing, curated catalog
  (`railwise mcp list`), bundled demo server.
- **`railwise setup` wizard**: API key → preset pick → MCP multi-select
  → per-server args → `~/.reasonix/config.json`. `npx railwise` with
...
**Headline:** `railwise setup` replaces the CLI-flag maze. New users run
...
- **`railwise setup`** — interactive Ink wizard:
...
- **`railwise` with no subcommand** — launches the wizard on first run,
  drops straight into chat afterwards using saved defaults. Designed
  so a brand-new user can `npx railwise` and be chatting in 30s
...
- **`/setup` slash command** — prints instructions to exit and re-run
  `railwise setup`. Live reconfiguration mid-session is out of scope:
...
- `railwise chat` / `railwise run`: when a flag is not passed, the
...
- **One broken MCP server no longer kills the whole chat/run.** Before:
  ...
  `railwise setup` and the session continues with whatever succeeded.
...
not just local subprocesses. Pass a URL to `--mcp` and Railwise opens
...
- **`--mcp` now accepts URLs.** The parser routes anything starting
  ...
  stdio as before. Both namespaced and anonymous forms work:
    ```
    railwise chat --mcp "kb=https://mcp.example.com/sse"
    railwise run  --mcp "http://127.0.0.1:9000/sse" --task "..."
    ```
...
— `railwise mcp list` prints a curated catalog with copy-paste commands.
...
- **`railwise mcp list`** — curated catalog of popular official MCP
...
Verified end-to-end on live DeepSeek: `railwise run --mcp "..."` spawns an
...
- **`railwise chat --mcp "<cmd>"`** and **`railwise run --mcp "<cmd>"`** —
...
- **`bridgeMcpTools(client)`** — walk an MCP server's tools/list result
  and register each into a Railwise `ToolRegistry`. MCP tools become
...
**Headline:** 48-run bench data (3 repeats × 8 tasks × 2 modes). Railwise
...
- **t05 predicate relaxed** (`benchmarks/tau-bench/tasks.ts`). The task
  ...
  marking railwise as fail on its single run in v0.1; with this fix
  railwise now passes every refusal task in every repeat.
...
- **README headline numbers updated** to the 48-run set. Baseline shows
  ...
  skipped identity verification); Railwise held the guardrail on every
  run.
...
**Headline:** v0.2 grows eyes. `railwise replay` and `railwise diff` now
...
- **Interactive `railwise replay <transcript>`** — Ink TUI with
  ...
  aggregate.
- **Interactive `railwise diff <a> <b>`** — split-pane Ink TUI. Both
...
- **New CLI flags** on both commands:
  - `railwise replay --print` — force stdout pretty-print (auto when
    stdout isn't a TTY, or when `--head` / `--tail` is passed).
  - `railwise diff --print` — force stdout table.
  - `railwise diff --tui` — force Ink TUI even when piped (rare
    escape hatch).

### Changed

- **`railwise replay` default** is now the TUI. Old stdout behavior
  reachable via `--print` or by piping. Non-TTY detection
  automatically flips to stdout mode, so shell pipelines behave as
  they did in 0.2.0.
- **`railwise diff` default** picks itself from context:
...
- **`railwise replay <transcript>`** — pretty-print a past transcript and
  rebuild its full session summary (turns, tool calls, cache hit, cost,
  prefix stability) offline. No API calls.
- **`railwise diff <a> <b>`** — compare two transcripts: aggregate deltas,
...
- **`benchmarks/tau-bench/transcripts/`** — committed reference transcripts
  (baseline + railwise on `t01_address_happy`) so anyone can clone the
  repo and run `railwise replay` / `diff` immediately, without running
  the bench.
...
- **Transcript format bumped (backward-compatible)**. Records now carry
  `usage`, `cost`, `model`, `prefixHash` (railwise only), and `toolArgs`.
...
- **Live bench numbers** published in `benchmarks/tau-bench/report.md`:
  - cache hit: baseline 43.9% → railwise **94.3%** (+50.3pp)
  - cost/task: baseline $0.00278 → railwise **$0.00162** (−42%)
  - vs Claude Sonnet 4.6 (token-count estimate): **~96% cheaper**
  - pass rate: 100% (baseline) vs 88% (railwise; 1 predicate too strict,
    documented)
