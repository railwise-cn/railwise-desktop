...
export class SkillStore {
  ...
  createWithContent(
    ...
    if (scope === "project" && !this.projectRoot) {
      return { error: "project scope requires a workspace — run from `railwise code`" };
    }
    const root =
      scope === "project"
...
Key facts:
- QQ is a remote channel attached to an existing Railwise session, not a separate mode.
