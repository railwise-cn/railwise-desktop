...
export const de: TranslationSchema = {
  ...
  stats: {
    ...EN.stats,
    usageHint: "Führe `railwise chat`, `railwise code` oder `railwise run <task>` aus – jeden Turn",
    usageDetail: "Hängt eine Zeile an das Log an; `railwise stats` fasst sie zusammen.",
  },
  run: {
    ...EN.run,
    missingApiKey:
      "DEEPSEEK_API_KEY ist nicht gesetzt und stdin ist kein TTY (Nachfrage nicht möglich).\n" +
      "Setze die Umgebungsvariable oder starte einmal interaktiv `railwise chat`, um einen Schlüssel zu speichern.\n",
  },
  sessions: {
    ...EN.sessions,
    emptyHint:
      "Noch keine gespeicherten Sitzungen – starte `railwise chat` (Sitzungen werden automatisch gespeichert, außer mit --no-session).",
    listHeader: "Gespeicherte Sitzungen (~/.reasonix/sessions/):",
    inspectHint: "Ansehen:       railwise sessions <name>",
    resumeHint: "Fortsetzen:    railwise chat --session <name>",
  ...
  ui: {
    ...
    welcome:
      "Starte jederzeit `railwise`, um zu chatten – deine Einstellungen bleiben gespeichert.",
  ...
  code: {
    ...EN.code,
    workspaceConflict:
      "⚠ Arbeitsbereich enthält Dateien einer anderen Agent-Plattform ({platforms}). Railwise Code kann sie als Projektinhalt lesen; starte mit --dir <dein-projekt> neu, falls das nicht gewünscht ist.\n",
  ...
  slash: {
    ...
    setup: { ...EN.slash.setup, description: "Erinnert dich daran, `railwise setup` auszuführen" },
    semantic: {
      ...EN.slash.semantic,
      description:
  ...
  wizard: {
    ...
    mcpTitle: "Welche MCP-Server soll Railwise für dich einrichten?",
  ...
  errors: {
    ...EN.errors,
    contextOverflow:
      "Context-Überlauf (DeepSeek 400): Sitzungsverlauf ist {requested}, über dem Prompt-Limit des Modells (V4: 1M Tokens; legacy chat/reasoner: 131k). Meist ist ein einzelnes Tool-Ergebnis zu groß geworden. Railwise begrenzt neue Tool-Ergebnisse auf 8k Tokens und heilt überdimensionierte Verläufe automatisch beim Sitzungsladen – ein Neustart behebt es oft. Falls es weiterhin überläuft, führe /new für einen frischen Start aus oder öffne /sessions und drücke [d], um diese Sitzung zu löschen.",
    contextOverflowTooMany: "Zu viele Tokens",
    auth401:
      "Authentifizierung fehlgeschlagen (DeepSeek 401): {inner}. Dein API-Schlüssel wird abgewiesen. Behebe mit `railwise setup` oder `export DEEPSEEK_API_KEY=sk-...`. Erhalte einen unter https://platform.deepseek.com/api_keys.",
    balance402:
  ...
  handlers: {
    ...
    basic: {
      ...
      helpSessionCustom:
        "  railwise chat --session <name>   eine andere benannte Sitzung verwenden",
      helpSessionNone: "  railwise chat --no-session       Persistenz für diesen Lauf deaktivieren",
      ...
      aboutHeader: "Railwise v{version} — ein Cache-First-DeepSeek-Coding-Agent",
    ...
    admin: {
      ...
      updateCurrent: "Aktuell: railwise {version}",
      updateLatestPending:
      ...
      updateRetryHint2:
        "oder führe `railwise update` in einem anderen Terminal aus, um es synchron zu erzwingen.",
      updateLatest: "Neueste:  railwise {version}",
      updateUpToDate: "Du bist auf dem neuesten Stand. Nichts zu tun.",
      updateNpxHint:
        "Du verwendest npx — der nächste `npx railwise ...`-Start lädt automatisch die neueste Version.",
      ...
      updateUpgradeCmd1: "  railwise update           (interaktiv, --dry-run wird unterstützt)",
    ...
    edits: {
      ...EN.handlers.edits,
      undoCodeOnly:
        "/undo ist nur innerhalb von `railwise code` verfügbar — der Chat-Modus wendet keine Edits an.",
      historyCodeOnly: "/history ist nur innerhalb von `railwise code` verfügbar.",
      showCodeOnly: "/show ist nur innerhalb von `railwise code` verfügbar.",
      applyCodeOnly:
        "/apply ist nur innerhalb von `railwise code` verfügbar (hier gibt es nichts anzuwenden).",
      discardCodeOnly: "/discard ist nur innerhalb von `railwise code` verfügbar.",
      planCodeOnly:
        "/plan ist nur innerhalb von `railwise code` verfügbar — der Chat-Modus blockiert keine Tool-Schreibzugriffe.",
      planOn:
      ...
      modeCodeOnly: "/mode ist nur innerhalb von `railwise code` verfügbar.",
      ...
      commitCodeOnly:
        "/commit ist nur innerhalb von `railwise code` verfügbar (benötigt ein Git-Repo als Wurzel).",
      commitUsage:
        'Verwendung: /commit "deine Commit-Nachricht"  — führt `git add -A && git commit -m "…"` in {root} aus',
      walkCodeOnly: "/walk ist nur innerhalb von `railwise code` verfügbar.",
      checkpointCodeOnly:
        "/checkpoint ist nur innerhalb von `railwise code` verfügbar — der Chat-Modus wendet keine Edits an.",
      checkpointNone:
      ...
      restoreCodeOnly: "/restore ist nur innerhalb von `railwise code` verfügbar.",
      ...
      cwdCodeOnly: "/cwd ist nur innerhalb von `railwise code` verfügbar.",
      cwdUsage:
    ...
    model: {
      ...
      budgetNoCap:
        "Kein Sitzungsbudget festgelegt — Railwise wird weiterlaufen, bis du es stoppst. Setze eines mit: /budget <usd>   (z.B. /budget 5)",
      budgetStatus:
    ...
    permissions: {
      ...
      mutateCodeOnly:
        "/permissions add / remove / clear sind nur innerhalb von `railwise code` verfügbar — sie bearbeiten die projektbezogene Allowlist (`~/.reasonix/config.json` projects[<root>].shellAllowed).",
      addUsage:
    ...
    plans: {
      ...EN.handlers.plans,
      noSession:
        "Keine Sitzung angehängt — `/plans` ist pro Sitzung. Führe `railwise code` in einem Projekt aus, um eine Sitzung zu erhalten.",
      activePlan:
      ...
      replayNoSession:
        "Keine Sitzung angehängt — `/replay` ist pro Sitzung. Führe `railwise code` in einem Projekt aus, um eine Sitzung zu erhalten.",
      replayNoArchives:
    ...
    jobs: {
      ...EN.handlers.jobs,
      codeOnly: "/jobs ist nur innerhalb von `railwise code` verfügbar.",
      killCodeOnly: "/kill ist nur innerhalb von `railwise code` verfügbar.",
      logsCodeOnly: "/logs ist nur innerhalb von `railwise code` verfügbar.",
      empty:
    ...
    mcp: {
      ...EN.handlers.mcp,
      noServers:
        'Keine MCP-Server angehängt. Führe `railwise setup` aus, um welche auszuwählen, oder starte mit --mcp "<spec>". `railwise mcp list` zeigt den Katalog. Hinweis: vom Modell aufgerufene Shell-Befehle werden pro Aufruf abgefragt (einmal erlauben / immer erlauben / ablehnen) — kein globales Allow-All-Flag.',
      ...
      catalogHint:
        "Vollständiger Katalog: `railwise mcp list` · tiefere Diagnose: `railwise mcp inspect <spec>`.",
      fallbackServers: "MCP-Server ({count}):",
      fallbackTools: "Tools im Register ({count}):",
      fallbackChange: "Um diesen Satz zu ändern, beende und führe `railwise setup` aus.",
      usageDisableEnable:
    ...
    skill: {
      ...EN.handlers.skill,
      listEmpty: "Keine Skills gefunden. Railwise liest Skills von:",
      listProjectScope:
      ...
      listProjectOnly: "  (Projekt-Bereich ist nur in `railwise code` aktiv)",
  ...
  startup: {
    ...EN.startup,
    codeRooted:
      '▸ railwise code: verwurzelt in {rootDir}, Sitzung "{session}" · {tools} native Tool{s}{semantic}',
    ephemeral: "(ephemer)",
  ...
  mcpHealth: {
    ...
    emptyHint:
      "ℹ keine MCP-Server konfiguriert — versuche: `railwise setup` zur erneuten Auswahl, oder `railwise mcp install filesystem` · Shell-Befehle werden pro Aufruf abgefragt (einmal erlauben / immer erlauben / ablehnen), kein globales Allow-All",
  },
  denyContextInput: {
    ...EN.denyContextInput,
    description:
  ...
  mcpMarketplace: {
    ...
    bridgeReloadFailed:
      "✓ {name} installiert - starte `railwise code` neu zur Verbindung (Neuladen fehlgeschlagen: {message})",
    restartBridge: "✓ {name} installiert - starte `railwise code` neu zur Verbindung",
  ...
  mcpBrowser: {
    ...
    title: "◈ MCP-Browser",
    empty:
      "Keine MCP-Server angehängt. Führe `railwise setup` aus, um welche auszuwählen, oder starte mit --mcp.",
  ...
  mcpLifecycle: {
    ...
    failedSetupHint:
      "→ führe `railwise setup` aus, um diesen Eintrag zu entfernen, oder behebe das zugrunde liegende Problem (fehlendes npm-Paket, Netzwerk usw.).",
    failedSetupConfigHint:
      "→ führe `railwise setup` aus, um fehlerhafte Einträge aus deiner gespeicherten Konfiguration zu entfernen.",
    abortedHint:
  ...
  mcpCli: {
    ...
    morePagesAvailable: "▸ mehr Seiten verfügbar — `railwise mcp list --pages <n>` oder --all",
    installHint: "Installieren:  railwise mcp install <name>",
    usageSearch: "Verwendung: railwise mcp search <abfrage>",
    usageInstall: "Verwendung: railwise mcp install <name>",
    ...
    moreLoaded: "… {count} weitere geladen — verwende `railwise mcp search <abfrage>` zum Filtern",
    moreMatches: "… {count} weitere Treffer",
    installed: "Installiert: {spec}",
    noServerFound:
      'Kein MCP-Server namens "{target}" gefunden nach {pages} Seite(n) der {source}-Registry.',
    noServerTryMore: "Versuche: railwise mcp install {target} --max-pages 100",
    noInstallMeta:
