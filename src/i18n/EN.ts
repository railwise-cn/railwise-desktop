...
export const EN: TranslationSchema = {
  ...
  cli: {
    ...
    version: "Print Railwise version.",
    update: "Check for a newer Railwise and install it.",
    index: "Build (or incrementally refresh) a local semantic search index.",
  },
  stats: {
    usageHint: "run `railwise chat`, `railwise code`, or `railwise run <task>` — every turn",
    usageDetail: "appends one line to the log and `railwise stats` will roll it up.",
  },
  run: {
    missingApiKey:
      "DEEPSEEK_API_KEY is not set and stdin is not a TTY (cannot prompt).\n" +
      "Set the env var, or run `railwise chat` once interactively to save a key.\n",
  },
  sessions: {
    emptyHint:
      "no saved sessions yet — run `railwise chat` (sessions are auto-saved unless --no-session).",
    listHeader: "Saved sessions (~/.reasonix/sessions/):",
    inspectHint: "Inspect:  railwise sessions <name>",
    resumeHint: "Resume:   railwise chat --session <name>",
  ...
  ui: {
    welcome: "Run `railwise` any time to start chatting — your settings are remembered.",
    ...
    keysReference: {
      topic: "Railwise keys + mouse reference",
      sections: [
        {
          title: "keyboard",
          rows: [
  ...
  code: {
    workspaceConflict:
      "⚠ workspace contains another agent platform's files ({platforms}). Railwise Code may read them as project content; relaunch with --dir <your-project> if that's not what you want.\n",
  ...
  slash: {
    ...
    setup: { description: "reminds you to exit and run `railwise setup`" },
    semantic: {
  ...
  wizard: {
    ...
    themeCaption: {
      ...
      ember: "Warm dark palette with stronger Railwise orange accents",
    ...
    mcpTitle: "Which MCP servers should Railwise wire up for you?",
  ...
  errors: {
    ...
    upstream5xxHead:
      "Upstream service unavailable ({status}) at {host} — the configured API endpoint returned a server error, not a Railwise bug. Already retried 4× with backoff.",
  ...
  handlers: {
    basic: {
      ...
      helpSessionCustom: "  railwise chat --session <name>   use a different named session",
      helpSessionNone: "  railwise chat --no-session       disable persistence for this run",
      ...
      aboutHeader: "Railwise v{version} — a cache-first DeepSeek coding agent",
    ...
    admin: {
      ...
      updateCurrent: "current: railwise {version}",
      ...
      updateRetryHint2: "or run `railwise update` in another terminal to force it synchronously.",
      updateLatest: "latest:  railwise {version}",
      updateUpToDate: "you're on the latest. nothing to do.",
      updateNpxHint: "you're running via npx — the next `npx railwise ...` launch will auto-fetch.",
      updateNpxForce: "to force a refresh sooner: `npm cache clean --force`.",
      updateUpgradeHint: "to upgrade, exit this session and run:",
      updateUpgradeCmd1:
        "  railwise update           (interactive, dry-run supported via --dry-run)",
    ...
    edits: {
      undoCodeOnly:
        "/undo is only available inside `railwise code` — chat mode doesn't apply edits.",
      historyCodeOnly: "/history is only available inside `railwise code`.",
      showCodeOnly: "/show is only available inside `railwise code`.",
      applyCodeOnly: "/apply is only available inside `railwise code` (nothing to apply here).",
      discardCodeOnly: "/discard is only available inside `railwise code`.",
      planCodeOnly:
        "/plan is only available inside `railwise code` — chat mode doesn't gate tool writes.",
      planOn:
      ...
      modeCodeOnly: "/mode is only available inside `railwise code`.",
      ...
      commitCodeOnly: "/commit is only available inside `railwise code` (needs a rooted git repo).",
      commitUsage:
        'usage: /commit "your commit message"  — runs `git add -A && git commit -m "…"` in {root}',
      walkCodeOnly: "/walk is only available inside `railwise code`.",
      checkpointCodeOnly:
        "/checkpoint is only available inside `railwise code` — chat mode doesn't apply edits.",
      checkpointNone:
      ...
      restoreCodeOnly: "/restore is only available inside `railwise code`.",
      ...
      cwdCodeOnly: "/cwd is only available inside `railwise code`.",
      cwdUsage:
    ...
    model: {
      ...
      budgetNoCap:
        "no session budget set — Railwise will keep going until you stop it. Set one with: /budget <usd>   (e.g. /budget 5)",
      budgetStatus:
    ...
    permissions: {
      mutateCodeOnly:
        "/permissions add / remove / clear are only available inside `railwise code` — they edit the project-scoped allowlist (`~/.reasonix/config.json` projects[<root>].shellAllowed).",
      addUsage:
    ...
    plans: {
      noSession:
        "no session attached — `/plans` is per-session. Run `railwise code` in a project to get a session.",
      ...
      replayNoSession:
        "no session attached — `/replay` is per-session. Run `railwise code` in a project to get a session.",
      replayNoArchives:
    ...
    jobs: {
      codeOnly: "/jobs is only available inside `railwise code`.",
      killCodeOnly: "/kill is only available inside `railwise code`.",
      logsCodeOnly: "/logs is only available inside `railwise code`.",
      empty:
    ...
    mcp: {
      noServers:
        'no MCP servers attached. Run `railwise setup` to pick some, or launch with --mcp "<spec>". `railwise mcp list` shows the catalog. Note: model-invoked shell commands are gated per-call (allow once / allow always / deny) — no global allow-all flag.',
      ...
      catalogHint:
        "Full catalog: `railwise mcp list` · deeper diagnosis: `railwise mcp inspect <spec>`.",
      fallbackServers: "MCP servers ({count}):",
      fallbackTools: "Tools in registry ({count}):",
      fallbackChange: "To change this set, exit and run `railwise setup`.",
      usageDisableEnable:
    ...
    skill: {
      listEmpty: "no skills found. Railwise reads skills from:",
      listProjectScope:
      ...
      listProjectOnly: "  (project scope is only active in `railwise code`)",
  ...
  startup: {
    codeRooted:
      '\u25b8 railwise code: rooted at {rootDir}, session "{session}" \u00b7 {tools} native tool(s){semantic}',
    ephemeral: "(ephemeral)",
  ...
  mcpMarketplace: {
    ...
    bridgeReloadFailed:
      "\u2713 installed {name} - restart `railwise code` to bridge (reload failed: {message})",
    restartBridge: "\u2713 installed {name} - restart `railwise code` to bridge",
  ...
  mcpBrowser: {
    title: "\u25c8 MCP browser",
    empty: "No MCP servers attached. Run `railwise setup` to pick some, or launch with --mcp.",
  ...
  mcpLifecycle: {
    ...
    failedSetupHint:
      "→ run `railwise setup` to remove this entry, or fix the underlying issue (missing npm package, network, etc.).",
    failedSetupConfigHint:
      "→ run `railwise setup` to remove broken entries from your saved config.",
    abortedHint:
  ...
  mcpCli: {
    ...
    morePagesAvailable:
      "\u25b8 more pages available \u2014 `railwise mcp list --pages <n>` or --all",
    installHint: "Install:  railwise mcp install <name>",
    usageSearch: "usage: railwise mcp search <query>",
    usageInstall: "usage: railwise mcp install <name>",
    ...
    moreLoaded: "\u2026 {count} more loaded \u2014 use `railwise mcp search <query>` to filter",
    moreMatches: "\u2026 {count} more matches",
    installed: "Installed: {spec}",
    noServerFound:
      'No MCP server named "{target}" found after walking {pages} page(s) of the {source} registry.',
    noServerTryMore: "Try: railwise mcp install {target} --max-pages 100",
    noInstallMeta:
