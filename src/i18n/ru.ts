...
export const ru: TranslationSchema = {
  ...
  cli: {
    ...
    version: "Вывести версию Railwise.",
    update: "Проверить новую версию Railwise и установить.",
    index: "Построить (или инкрементально обновить) локальный семантический поисковый индекс.",
  },
  stats: {
    ...EN.stats,
    usageHint: "запусти `railwise chat`, `railwise code` или `railwise run <task>` — каждый шаг",
    usageDetail: "добавляет одну строку в лог, а `railwise stats` сводит всё вместе.",
  },
  run: {
    ...EN.run,
    missingApiKey:
      "DEEPSEEK_API_KEY не задан и stdin не TTY (нельзя запросить).\n" +
      "Установи переменную окружения или запусти `railwise chat` один раз для сохранения ключа.\n",
  },
  sessions: {
    ...EN.sessions,
    emptyHint:
      "ещё нет сохранённых сессий — запусти `railwise chat` (сессии авто-сохраняются, если не указан --no-session).",
    listHeader: "Сохранённые сессии (~/.reasonix/sessions/):",
    inspectHint: "Просмотр:  railwise sessions <имя>",
    resumeHint: "Возобновить: railwise chat --session <имя>",
  ...
  ui: {
    ...
    welcome: "Запускай `railwise` в любое время — настройки сохраняются.",
  ...
  code: {
    ...EN.code,
    workspaceConflict:
      "⚠ рабочая область содержит файлы другой платформы агента ({platforms}). Railwise Code может прочитать их как содержимое проекта; перезапусти с --dir <твой-проект> если это нежелательно.\n",
  ...
  slash: {
    ...
    setup: { ...EN.slash.setup, description: "напоминает выйти и запустить `railwise setup`" },
    semantic: {
      ...EN.slash.semantic,
  ...
  wizard: {
    ...
    welcomeTitle: "Добро пожаловать в Railwise.",
    ...
    themeCaption: {
      ...
      ember: "Тёплая тёмная тема с более ярким оранжевым акцентом Railwise",
    ...
    mcpTitle: "Какие MCP-серверы должен подключить Railwise?",
