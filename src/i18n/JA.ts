...
export const JA: TranslationSchema = {
  ...
  cli: {
    ...
    version: "Railwise のバージョンを表示します。",
    update: "新しい Railwise をチェックしてインストールします。",
    index: "ローカルのセマンティック検索インデックスを構築（または増分更新）します。",
  },
  stats: {
    usageHint: "`railwise chat`、`railwise code`、または `railwise run <task>` を実行するたびに",
    usageDetail: "ログに1行追加され、`railwise stats` で集計されます。",
  },
  run: {
    missingApiKey:
      "DEEPSEEK_API_KEY が設定されておらず、stdin が TTY ではありません（プロンプトを表示できません）。\n" +
      "環境変数を設定するか、`railwise chat` を一度対話的に実行してキーを保存してください。\n",
  },
  sessions: {
    emptyHint:
      "保存されたセッションはまだありません — `railwise chat` を実行してください（--no-session を付けない限り自動保存されます）。",
    listHeader: "保存されたセッション (~/.reasonix/sessions/):",
    inspectHint: "詳細表示:  railwise sessions <name>",
    resumeHint: "再開:   railwise chat --session <name>",
  ...
  code: {
    workspaceConflict:
      "⚠ ワークスペースに別のエージェントプラットフォームのファイル ({platforms}) が含まれています。Railwise Code がそれらをプロジェクトコンテンツとして読み取る可能性があります。意図しない場合は --dir <your-project> で再起動してください。\n",
  ...
  slash: {
    ...
    setup: { description: "終了して `railwise setup` を実行するよう促します" },
    semantic: {
  ...
  wizard: {
    ...
    themeCaption: {
      ...
      ember: "Railwiseのオレンジを強めた暖かいダークテーマ",
      ...
      dark: "クールなダークトーン（旧別名）",
      light: "クリーンなライトモード（旧別名）",
      "deep-blue": "黒背景にディープブルー（旧別名）",
      "high-contrast": "アクセシビリティ（旧別名）",
    },
    reviewLabelTheme: "テーマ",
    mcpTitle: "Railwise に接続するMCPサーバーを選択してください",
  ...
  errors: {
    contextOverflow:
      "コンテキストオーバーフロー (DeepSeek 400): セッション履歴が {requested} で、モデルのプロンプト制限（V4: 1Mトークン; 旧来のchat/reasoner: 131k）を超えています。通常、単一のツール結果が大きすぎるのが原因です。Railwiseは新しいツール結果を8kトークンに制限し、セッション読み込み時に大きすぎる履歴を自動修復します — 再起動で解決することが多いです。それでもオーバーフローする場合は、/new で新規開始するか、/sessions を開いて [d] でこのセッションを削除してください。",
    contextOverflowTooMany: "トークンが多すぎます",
    auth401:
      "認証に失敗しました (DeepSeek 401): {inner}。APIキーが拒否されました。`railwise setup` または `export DEEPSEEK_API_KEY=sk-...` で修正してください。https://platform.deepseek.com/api_keys から取得できます。",
    balance402:
    ...
    concurrency429:
      "DeepSeekの同時実行制限に達しました (429): {inner}。アカウントの処理中リクエストが多すぎます（上限: v4-proは500、v4-flashは2500、アカウント全体のAPIキー合計）。通常、別のRailwiseプロセスが同じキーを共有しているか、並列サブエージェントのファンアウトが超過したことを意味します。数秒待って再試行するか、並列度を下げるか、https://platform.deepseek.com でより高い上限をリクエストしてください。",
    deepseek5xxHead:
      "DeepSeekサービス利用不可 ({status}) — これはDeepSeek側の問題であり、Railwiseの問題ではありません。バックオフ付きで4回再試行済みです。",
    ...
    upstream5xxHead:
      "アップストリームサービス利用不可 ({status}) at {host} — 設定されたAPIエンドポイントがサーバーエラーを返しました。Railwiseのバグではありません。バックオフ付きで4回再試行済みです。",
  ...
  handlers: {
    basic: {
      ...
      helpSessionCustom: "  railwise chat --session <name>   別の名前付きセッションを使用",
      helpSessionNone: "  railwise chat --no-session       この実行の永続化を無効化",
      retryNone:
      ...
      aboutHeader: "Railwise v{version} — キャッシュファーストのDeepSeekコーディングエージェント",
    ...
    edits: {
      undoCodeOnly:
        "/undo は `railwise code` 内でのみ利用可能です — チャットモードでは編集を適用しません。",
      historyCodeOnly: "/history は `railwise code` 内でのみ利用可能です。",
      showCodeOnly: "/show は `railwise code` 内でのみ利用可能です。",
      applyCodeOnly:
        "/apply は `railwise code` 内でのみ利用可能です（ここで適用するものはありません）。",
      discardCodeOnly: "/discard は `railwise code` 内でのみ利用可能です。",
      planCodeOnly:
        "/plan は `railwise code` 内でのみ利用可能です — チャットモードではツール書き込みを制御しません。",
      planOn:
      ...
      modeCodeOnly: "/mode は `railwise code` 内でのみ利用可能です。",
      ...
      modeReview:
        "▸ 編集モード: review — 編集は /apply（または y）/ /discard（または n）のキューに入ります",
      commitCodeOnly:
        "/commit は `railwise code` 内でのみ利用可能です（git リポジトリが必要です）。",
      commitUsage:
        '使い方: /commit "コミットメッセージ"  — {root} で `git add -A && git commit -m "…"` を実行します',
      walkCodeOnly: "/walk は `railwise code` 内でのみ利用可能です。",
      checkpointCodeOnly:
        "/checkpoint は `railwise code` 内でのみ利用可能です — チャットモードでは編集を適用しません。",
      checkpointNone:
      ...
      restoreCodeOnly: "/restore は `railwise code` 内でのみ利用可能です。",
      ...
      cwdCodeOnly: "/cwd は `railwise code` 内でのみ利用可能です。",
      cwdUsage:
    ...
    permissions: {
      mutateCodeOnly:
        "/permissions add / remove / clear は `railwise code` 内でのみ利用可能です — プロジェクトスコープの許可リストを編集します（`~/.reasonix/config.json` projects[<root>].shellAllowed）。",
      addUsage:
    ...
    plans: {
      noSession:
        "セッションが接続されていません — `/plans` はセッションごとです。プロジェクトで `railwise code` を実行するとセッションが取得できます。",
      ...
      replayNoSession:
        "セッションが接続されていません — `/replay` はセッションごとです。プロジェクトで `railwise code` を実行するとセッションが取得できます。",
      replayNoArchives:
    ...
    jobs: {
      codeOnly: "/jobs は `railwise code` 内でのみ利用可能です。",
      killCodeOnly: "/kill は `railwise code` 内でのみ利用可能です。",
      logsCodeOnly: "/logs は `railwise code` 内でのみ利用可能です。",
      empty:
    ...
    mcp: {
      noServers:
        'MCPサーバーが接続されていません。`railwise setup` を実行して選択するか、--mcp "<spec>" で起動してください。`railwise mcp list` でカタログを表示します。注意: モデル起動のシェルコマンドは呼び出しごとに制御されます（allow once / allow always / deny）— 全許可フラグはありません。',
      ...
      catalogHint:
        "完全なカタログ: `railwise mcp list` · 詳細診断: `railwise mcp inspect <spec>`。",
      fallbackServers: "MCPサーバー ({count}):",
      fallbackTools: "登録済みツール ({count}):",
      fallbackChange: "このセットを変更するには、終了して `railwise setup` を実行してください。",
      usageDisableEnable:
    ...
    init: {
      codeOnly:
        "/init はコードモードでのみ動作します（ファイルシステムツールが必要です）。\n初期化したいプロジェクトをルートとして `railwise code [path]` でセッションを開始し、/init を実行してください。",
  ...
  startup: {
    codeRooted:
      '\u25b8 railwise code: ルート {rootDir}, セッション "{session}" \u00b7 {tools} ネイティブツール{semantic}',
    ephemeral: "（エフェメラル）",
  ...
  mcpHealth: {
    ...
    emptyHint:
      "\u2139 MCPサーバーが設定されていません \u2014 対処: `railwise setup` で再選択するか、`railwise mcp install filesystem` \u00b7 シェルコマンドは毎回確認（allow once / allow always / deny）、全許可フラグなし",
  },
  denyContextInput: {
    description:
  ...
  mcpMarketplace: {
    ...
    bridgeReloadFailed:
      "\u2713 {name} をインストール - `railwise code` を再起動してブリッジ（再読込失敗: {message}）",
    restartBridge: "\u2713 {name} をインストール - `railwise code` を再起動してブリッジ",
    needsEnv: "  \u00b7  要環境変数: {env}",
    badgeOfficial: "[公式]",
    badgeSmithery: "[smt]",
    badgeLocal: "[ローカル]",
  ...
  mcpBrowser: {
    title: "\u25c8 MCP ブラウザ",
    empty:
      "MCPサーバーが接続されていません。`railwise setup` を実行して選択するか、--mcp で起動してください。",
  ...
  mcpLifecycle: {
    ...
    failedSetupHint:
      "→ `railwise setup` を実行してこのエントリを削除するか、根本的な問題（npmパッケージ不足、ネットワークなど）を修正してください。",
    failedSetupConfigHint:
      "→ `railwise setup` を実行して、保存された設定から壊れたエントリを削除してください。",
    abortedHint:
  ...
  mcpCli: {
    ...
    allLoaded: "全件読み込み済み",
    morePagesAvailable:
      "\u25b8 さらにページがあります \u2014 `railwise mcp list --pages <n>` または --all",
    installHint: "インストール:  railwise mcp install <name>",
    usageSearch: "使い方: railwise mcp search <query>",
    usageInstall: "使い方: railwise mcp install <name>",
    noMatchesFor: '{count} 件の読み込み済みエントリ中に "{q}" の一致はありません（{source}）',
    matchCount: '{source} レジストリで "{q}" に {count} 件一致（{loaded} エントリをスキャン）:',
    moreLoaded:
      "\u2026 さらに {count} 件読み込み済み \u2014 `railwise mcp search <query>` でフィルタしてください",
    moreMatches: "\u2026 さらに {count} 件一致",
    installed: "インストール済み: {spec}",
    noServerFound:
      '{source} レジストリの {pages} ページを走査しましたが、"{target}" という名前のMCPサーバーは見つかりませんでした。',
    noServerTryMore: "試す: railwise mcp install {target} --max-pages 100",
    noInstallMeta:
