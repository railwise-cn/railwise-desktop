...
export const zhCN: TranslationSchema = {
  ...
  cli: {
    ...
    version: "打印 Railwise 版本。",
    update: "检查较新版本的 Railwise 并安装。",
    index: "构建（或增量刷新）本地语义搜索索引。",
  },
  stats: {
    usageHint: "运行 `railwise chat`、`railwise code` 或 `railwise run <task>` — 每次对话都会记录",
    usageDetail: "每次对话在日志中追加一行，`railwise stats` 会将其汇总统计。",
  },
  run: {
    missingApiKey:
      "未设置 DEEPSEEK_API_KEY 且标准输入不是 TTY（无法交互式输入）。\n" +
      "请设置环境变量，或先运行 `railwise chat` 交互一次以保存密钥。\n",
  },
  sessions: {
    emptyHint:
      "暂无已保存的会话 — 运行 `railwise chat`（会话会自动保存，除非使用了 --no-session）。",
    listHeader: "保存的会话 (~/.reasonix/sessions/)：",
    inspectHint: "查看：railwise sessions <name>",
    resumeHint: "恢复：railwise chat --session <name>",
  ...
  ui: {
    welcome: "随时运行 `railwise` 开始聊天 — 您的设置将被记住。",
  ...
  code: {
    workspaceConflict:
      "⚠ 工作区包含另一个智能体平台的文件 ({platforms})。Railwise Code 可能会将其作为项目内容读取；如果不是您想要的，请使用 --dir <your-project> 重新启动。\n",
  ...
  slash: {
    ...
    setup: { description: "提醒您退出并运行 `railwise setup`" },
    semantic: {
  ...
  wizard: {
    ...
    themeCaption: {
      ...
      ember: "暖黑深色主题，强化 Railwise 橙色品牌感",
    ...
    reviewLabelTheme: "主题",
    mcpTitle: "Railwise 要为你接入哪些 MCP 服务器？",
  ...
  errors: {
    contextOverflow:
      "上下文溢出（DeepSeek 400）：会话历史已达 {requested}，超出模型 prompt 上限（V4：1M tokens；旧版 chat/reasoner：131k）。通常是单个工具结果太大。Railwise 默认将新工具结果限制在 8k tokens，并在会话加载时自动修复超大历史 — 重启常能清掉。如果仍然溢出，运行 /new 重新开始，或打开 /sessions 选中后按 [d] 删除该会话。",
    contextOverflowTooMany: "tokens 数量过多",
    auth401:
      "认证失败（DeepSeek 401）：{inner}。你的 API key 被拒绝。运行 `railwise setup` 或 `export DEEPSEEK_API_KEY=sk-...` 修复。在 https://platform.deepseek.com/api_keys 获取 key。",
    balance402:
    ...
    concurrency429:
      "DeepSeek 并发超限（429）：{inner}。账号在跑的请求超过上限（v4-pro 500、v4-flash 2500，账号下所有 API key 累加）。通常是同一账号开了多个 Railwise 进程，或者并行 subagent 一次发太多。等几秒重试、减少并行，或在 https://platform.deepseek.com 申请扩容。",
    deepseek5xxHead:
      "DeepSeek 服务不可用（{status}） — 这是 DeepSeek 服务端问题，不是 Railwise 故障。已按指数退避重试 4 次。",
    ...
    upstream5xxHead:
      "上游服务不可用（{status}），目标地址 {host} — 你配置的 API 端点返回了服务器错误，不是 Railwise 故障。已按指数退避重试 4 次。",
  ...
  handlers: {
    basic: {
      ...
      helpSessionCustom: "  railwise chat --session <name>   使用不同的命名会话",
      helpSessionNone: "  railwise chat --no-session       禁用本次运行的持久化",
      ...
      aboutHeader: "Railwise v{version} — 缓存优先的 DeepSeek 编码代理",
    ...
    edits: {
      undoCodeOnly: "/undo 仅在 `railwise code` 中可用 — 聊天模式不应用编辑。",
      historyCodeOnly: "/history 仅在 `railwise code` 中可用。",
      showCodeOnly: "/show 仅在 `railwise code` 中可用。",
      applyCodeOnly: "/apply 仅在 `railwise code` 中可用（此处无内容可应用）。",
      discardCodeOnly: "/discard 仅在 `railwise code` 中可用。",
      planCodeOnly: "/plan 仅在 `railwise code` 中可用 — 聊天模式不限制工具写入。",
      planOn:
      ...
      modeCodeOnly: "/mode 仅在 `railwise code` 中可用。",
      ...
      commitCodeOnly: "/commit 仅在 `railwise code` 中可用（需要有根的 git 仓库）。",
      commitUsage: '用法：/commit "提交消息"  — 在 {root} 中运行 `git add -A && git commit -m "…"`',
      walkCodeOnly: "/walk 仅在 `railwise code` 中可用。",
      checkpointCodeOnly: "/checkpoint 仅在 `railwise code` 中可用 — 聊天模式不应用编辑。",
      checkpointNone:
      ...
      restoreCodeOnly: "/restore 仅在 `railwise code` 中可用。",
      ...
      cwdCodeOnly: "/cwd 仅在 `railwise code` 中可用。",
      cwdUsage:
    ...
    model: {
      ...
      budgetNoCap:
        "未设置会话预算 — Railwise 将持续运行直到您停止。使用以下方式设置：/budget <usd>   （例如 /budget 5）",
    ...
    permissions: {
      mutateCodeOnly:
        "/permissions add / remove / clear 仅在 `railwise code` 中可用 — 它们编辑项目范围的允许列表（`~/.reasonix/config.json` projects[<root>].shellAllowed）。",
      addUsage:
    ...
    plans: {
      noSession: "未附加会话 — `/plans` 是按会话的。在项目中运行 `railwise code` 以获取会话。",
      ...
      replayNoSession:
        "未附加会话 — `/replay` 是按会话的。在项目中运行 `railwise code` 以获取会话。",
      replayNoArchives:
    ...
    jobs: {
      codeOnly: "/jobs 仅在 `railwise code` 中可用。",
      killCodeOnly: "/kill 仅在 `railwise code` 中可用。",
      logsCodeOnly: "/logs 仅在 `railwise code` 中可用。",
      empty:
    ...
    mcp: {
      noServers:
        '未附加 MCP 服务器。运行 `railwise setup` 选择一些，或使用 --mcp "<spec>" 启动。`railwise mcp list` 显示目录。注：模型发起的 shell 命令按次审批（allow once / allow always / deny），设计上没有「全局放行」开关。',
      toolsLabel: "  工具     {count}",
      resourcesHint: "`/resource` 浏览+读取",
      promptsHint: "`/prompt` 浏览+获取",
      awarenessOnly: "聊天模式目前消耗工具；资源+提示在此展示供了解。",
      catalogHint: "完整目录：`railwise mcp list` · 深度诊断：`railwise mcp inspect <spec>`。",
      fallbackServers: "MCP 服务器（{count}）：",
      fallbackTools: "注册表中的工具（{count}）：",
      fallbackChange: "要更改此设置，请退出并运行 `railwise setup`。",
      usageDisableEnable:
    ...
    init: {
      codeOnly:
        "/init 仅在代码模式下工作（需要文件系统工具）。\n运行 `railwise code [path]` 启动一个以您要初始化的项目为根的会话，\n然后运行 /init。",
  ...
  startup: {
    codeRooted:
      '▸ railwise code：根目录 {rootDir}，会话 "{session}" · {tools} 个原生工具{semantic}',
    ephemeral: "（临时）",
    semanticOn: " · 语义搜索已开启",
  },
  doctorErrors: {
  ...
  mcpMarketplace: {
    ...
    bridgeReloadFailed: "✓ 已安装 {name} — 重启 `railwise code` 以桥接（重载失败：{message}）",
    restartBridge: "✓ 已安装 {name} — 重启 `railwise code` 以桥接",
    needsEnv: "  ·  需要环境变量：{env}",
    badgeOfficial: "[官方]",
    badgeSmithery: "[三方]",
    badgeLocal: "[本地]",
  ...
  mcpBrowser: {
    title: "◈ MCP 浏览器",
    empty: "没有挂载 MCP 服务器。运行 `railwise setup` 选择一些，或使用 --mcp 启动。",
  ...
  mcpLifecycle: {
    ...
    failedSetupHint: "→ 运行 `railwise setup` 移除此条目，或修复底层问题（缺少 npm 包、网络等）。",
    failedSetupConfigHint: "→ 运行 `railwise setup` 从已保存配置中移除损坏的条目。",
  ...
  mcpCli: {
    ...
    moreAvailable: "还有更多",
    allLoaded: "已全部加载",
    morePagesAvailable: "▸ 还有更多页可用 — `railwise mcp list --pages <n>` 或 --all",
    installHint: "安装：railwise mcp install <name>",
    usageSearch: "用法：railwise mcp search <query>",
    usageInstall: "用法：railwise mcp install <name>",
    noMatchesFor: '未找到 "{q}" 的匹配项（已检索 {count} 条记录，来源：{source}）',
    matchCount: '在 {source} 中找到 {count} 条 "{q}" 的匹配项（已扫描 {loaded} 条记录）：',
    moreLoaded: "… 还有 {count} 条已加载 — 使用 `railwise mcp search <query>` 筛选",
    moreMatches: "… 还有 {count} 条匹配项",
    installed: "已安装：{spec}",
    noServerFound: '在 {source} 中遍历了 {pages} 页后未找到名为 "{target}" 的 MCP 服务器。',
    noServerTryMore: "试试：railwise mcp install {target} --max-pages 100",
