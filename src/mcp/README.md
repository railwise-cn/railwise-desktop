...
client, hand-rolled in TypeScript. Lets Railwise consume tools from any
...
- **Zero runtime deps** for this module. Consistent with Railwise's
  policy of owning the wire format where it matters.
- **Surface tuning**: we only implement what Railwise actually uses —
...
| feature | status | note |
|---|---|---|
| CLI wiring (`railwise chat --mcp <cmd>`) | ✅ shipped | see Usage below |
...
| Resources / `resources/list` / `resources/read` | deferred | Railwise doesn't surface resources today |
...
| MCP server that Railwise exposes | never | out of scope — Railwise is a client |
...
railwise chat --mcp "node --import tsx examples/mcp-server-demo.ts"

# Official filesystem server:
railwise chat --mcp "npx -y @modelcontextprotocol/server-filesystem /tmp/safe-dir"
...
railwise run "list files in /tmp/safe-dir" \
...
`ImmutablePrefix` for the model, and every call goes through Railwise's
...
// 2. Bridge its tools into a Railwise ToolRegistry
...
The payoff: the filesystem server's tools now inherit Railwise's
