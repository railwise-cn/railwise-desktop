 ...
 * want a fresh check can run `railwise update` which passes
 * `force: true`. */
...
/** `name === "railwise"` guard avoids picking up an outer package.json when loaded as a dep. */
  ...
  try {
    ...
    for (let i = 0; i < 6; i++) {
      ...
      if (existsSync(p)) {
        ...
        if (pkg?.name === "railwise" && typeof pkg.version === "string") {
          return pkg.version;
        }
      }
...
export interface GetLatestVersionOptions {
  /** Ignore the cached entry and always fetch fresh. Used by `railwise update`. */
  force?: boolean;
...
export function detectInstallSource(bin?: string): InstallSource {
  ...
  if (/\/node_modules\/(?:reasonix|railwise)(\b|\/)/.test(norm)) return "npm";
  return "unknown";
}
...
export function detectNpmInstallPrefix(bin?: string): string | null {
  ...
  const posix = norm.match(/^(.+?)\/lib\/node_modules\/(?:reasonix|railwise)(?:\/|$)/i);
  if (posix) return posix[1] ?? null;
  const win = norm.match(/^(.+?)\/node_modules\/(?:reasonix|railwise)(?:\/|$)/i);
