...
export async function runDoctorChecks(projectRoot: string): Promise<DoctorCheck[]> {
  ...
  return [
    ...
    r[6],
    r[7],
    ...runRailwiseReadinessChecks(projectRoot),
  ];
}
    ...
    return [
      {
        ...
        label: "http proxy   ",
        level: "ok",
        detail: `${urlSource}=${redacted} is set but cfg.proxy.disabled — Railwise routes direct`,
      },
    ];
  }
...
async function checkApiKey(): Promise<Check> {
  ...
  return {
    ...
    level: "fail",
    detail:
      "not set — `railwise setup` to save one, or export DEEPSEEK_API_KEY. Get a key at https://platform.deepseek.com/api_keys",
  };
}
...
async function checkConfig(): Promise<Check> {
  ...
  if (!existsSync(path)) {
    return {
      ...
      label: "config       ",
      level: "warn",
      detail: "missing — running with library defaults. `railwise setup` writes one.",
    };
  }
  try {
  ...
  try {
    ...
    if (stale > 0) {
      return {
        ...
        label: "sessions     ",
        level: "warn",
        detail: `${detail} · ${stale} idle ≥90d (run \`railwise prune-sessions\`)`,
      };
    }
...
async function checkOllama(projectRoot: string): Promise<Check> {
  ...
  if (!exists) {
    return {
      ...
      label: "semantic     ",
      level: "ok",
      detail: "not in use (no semantic index built; `railwise index` to enable)",
    };
  }
...
async function checkProject(projectRoot: string): Promise<Check> {
  ...
  // package.json. Lacking all three, `railwise code` still works but
  ...
  if (found.length === 0) {
    return {
      ...
      label: "project      ",
      level: "warn",
      detail: `${projectRoot} has none of: ${markers.slice(0, 3).join(", ")} … — \`railwise code\` will still run, but @-mentions and project memory have nothing to anchor`,
    };
  }
  return {
    id: "project",
...
export async function doctorCommand(opts: DoctorOptions = {}): Promise<void> {
  ...
  if (!json) {
    console.log(`${color(`railwise ${VERSION}  ·  doctor`, "1")}  (cwd: ${projectRoot})`);
