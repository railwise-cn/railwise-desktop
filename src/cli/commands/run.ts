...
export interface RunOptions {
  ...
  system: string;
  budgetUsd?: number;
  /** JSONL transcript path — lets `railwise replay` / `diff` audit this run. */
  transcript?: string;
...
export async function runCommand(opts: RunOptions): Promise<void> {
  ...
  if (normalizedSpecs.length > 0) {
    ...
    for (const spec of normalizedSpecs) {
      ...
      try {
        ...
        // Non-fatal — skip and continue, same as `railwise chat`. A
  ...
  try {
    for await (const ev of loop.step(opts.task)) {
      ...
      if (ev.role === "assistant_final" && ev.stats?.usage) {
        // `railwise run` is often used in CI / scripting — we want
        // those turns to show up in `railwise stats` too so the
  ...
  if (opts.transcript) {
    ...
    process.stdout.write(`  → npx railwise replay ${opts.transcript}\n`);
  }
