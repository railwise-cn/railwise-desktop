...
import { toApprovalPrompt } from "@railwise/core-utils";
import {
  type FileWithStats,
  listDirectory,
...
function defaultRailwiseWorkspace(): string | undefined {
  ...
  for (let i = 0; i < 8; i++) {
    ...
    if (existsSync(join(candidate, ".mcp.json"))) return candidate;
...
interface ConfirmRequiredEvent {
  ...
  command: string;
  prompt?: import("@railwise/core-utils").ApprovalPrompt;
}
...
/** The desktop sidecar is a long-running daemon — Tauri spawns this Node process once per app launch and pipes JSON over stdin/stdout. Without these handlers, any orphaned promise rejection (e.g. from an aborted turn whose cleanup races a session-switch — #1074) crashes the process with exit code 1, which the Tauri host surfaces as "railwise exited (code 1)" and a full reconnect cycle. Log loudly so we can find the underlying bug, but don't take the daemon down. */
