...
export const MANUAL_UPDATE_COMMANDS: readonly string[] = [
  "npm install -g railwise@latest",
  "bun add -g railwise",
  "pnpm add -g railwise@latest",
  "yarn global add railwise@latest",
];
...
export function planUpdate(input: PlanUpdateInput): UpdatePlan {
  ...
  if (diff === 0) {
    return { action: "up-to-date", message: `railwise ${input.current} is up to date.` };
  }
  if (input.installSource === "npx") {
    return {
      action: "npx-hint",
      message: [
        `railwise ${input.latest} is available.`,
        "you're running via npx — the next `npx railwise ...` launch will auto-fetch",
  ...
  if (input.installSource === "unknown") {
    return {
      action: "manual-hint",
      message: [
        `railwise ${input.latest} is available, but the install source could not be determined automatically.`,
        "run one of these manually based on how you installed railwise:",
  ...
  return {
    action: "run-install",
    message: `upgrading railwise ${input.current} → ${input.latest} (via ${input.installSource})`,
    command,
  };
}
...
function buildUpdateCommand(
  ...
  switch (source) {
    case "npm":
      return npmPrefix
        ? ["npm", "--prefix", npmPrefix, "install", "-g", "railwise@latest"]
        : ["npm", "install", "-g", "railwise@latest"];
    case "bun":
      return ["bun", "add", "-g", "railwise"];
    case "pnpm":
      return ["pnpm", "add", "-g", "railwise@latest"];
    case "yarn":
      return ["yarn", "global", "add", "railwise@latest"];
  }
}
...
export async function updateCommand(opts: UpdateCommandOptions = {}): Promise<void> {
  ...
  write(`current: railwise ${VERSION}\n`);
