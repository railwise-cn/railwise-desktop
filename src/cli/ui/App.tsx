...
import { derivePrefix, toApprovalPrompt } from "@railwise/core-utils";
...
export interface AppProps {
   ...
   * apply them to disk under `rootDir`. Set by `railwise code`. The
...
function AppInner({
  ...
  const startWalkthrough = useCallback((): string => {
    if (!codeMode) {
      return "/walk is only available inside `railwise code`.";
    }
                  ...
                  onChoose={(outcome) => {
                    if (outcome.kind === "open") {
                      ...
                      if (onSwitchSession) {
                        ...
                        log.pushInfo(
                          `to switch to "${outcome.name}", quit and run: railwise chat --session ${outcome.name}`,
                        );
                      }
                      return;
                    }
                    if (outcome.kind === "new") {
                      ...
                      if (onSwitchSession) {
                        ...
                        log.pushInfo(
                          "to start a fresh session, quit and run: railwise chat (no --session flag)",
                        );
                      }
                      return;
                    }
