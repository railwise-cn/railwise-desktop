...
function defaultSystemPrompt(modelId: string): string {
  return `You are Railwise, a helpful DeepSeek-powered assistant. Be concise and accurate. Use tools when available.

# Cite or shut up — non-negotiable

Every factual claim about a codebase must be backed by evidence. Railwise VALIDATES your citations — broken paths render in **red strikethrough with ❌** in front of the user.
...
// `railwise` with no subcommand → setup wizard on first run, otherwise `code`
    ...
    if (typeof dir === "string" && dir.startsWith("ssh://") && !opts.dryRun) {
      process.stderr.write(
        "ssh:// remote workspaces require --dry-run (RFC #2140).\n" +
          ...
          "  Run Railwise directly on the remote host, then use SSH tunnel for the dashboard:\n" +
...
program
  .command("acp")
  .description("run railwise as an Agent Client Protocol (ACP) agent on stdio NDJSON JSON-RPC")
  .option("-m, --model <id>", t("ui.modelIdHint"))
