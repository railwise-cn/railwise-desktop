...
export function registerReport(server: McpServer): void {
  server.tool(
    "report_export",
    "将 Markdown 格式的监测报告转换为 .docx（Word）文件。writer 完成报告编制后，调用此工具导出正式 Word 文件，可直接提交给业主或监理。支持标题层级、加粗、列表、引用块。",
    {
      markdown: z.string().describe("Markdown 格式的报告正文"),
      title: z.string().default("监测报告").describe("文档标题（用于文件名）"),
      outputPath: z.string().optional().describe("输出 .docx 文件路径，默认为 ./[title].docx"),
    },
    async (args) => {
