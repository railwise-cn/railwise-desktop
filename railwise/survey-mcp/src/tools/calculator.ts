...
export function registerCalculator(server: McpServer): void {
  ...
  server.tool(
    ...
    async (args) => {
      ...
      const unknownIds = [
        ...new Set(args.observations.flatMap((o) => [o.from, o.to]).filter((id) => !knownMap.has(id))),
      ];
  ...
  server.tool(
    ...
    "附合导线/闭合导线坐标平差计算。输入起始点坐标、起始方位角、各站观测角和边长，返回平差后坐标、闭合差分析及各点精度。处理导线测量数据时必须调用此工具。",
    {
      ...
      startAzimuth: z.number().describe("起始边方位角（度，十进制）"),
      endAzimuth: z.number().describe("终止边方位角（度，十进制）；闭合导线传起始方位角"),
      stations: z
        .array(
          z.object({
