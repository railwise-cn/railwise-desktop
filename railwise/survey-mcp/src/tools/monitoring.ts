...
export function registerMonitoring(server: McpServer): void {
  server.tool(
    "monitoring_csv",
    "处理自动化监测仪器（静力水准、全站仪机器人、测斜仪）的海量CSV/TXT数据文件。输入文件路径，返回清洗后的核心数据指标（本期变化量、累计变化量、速率、超限测点列表）。data-analyst 必须调用此工具，绝不直接读取原始文件。",
    {
      filePath: z.string().describe("用户上传的 CSV、TXT 或 Excel 文件的绝对路径"),
      sensorType: z
    ...
    async (args) => {
      ...
      if (lines.length < 2) return ok({ error: "文件内容为空或仅有标题行，无有效数据。" });
      ...
      for (const row of rows) {
        ...
        pointGroups[id]!.push(v);
      }

      const totalPoints = Object.keys(pointGroups).length;
      if (totalPoints === 0) return ok({ error: "未能从文件中解析到有效的数值数据，请检查文件格式。" });
