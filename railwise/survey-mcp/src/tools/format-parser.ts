...
export function registerFormatParser(server: McpServer): void {
  server.tool(
    ...
    {
      filePath: z.string().describe("外业数据文件的绝对路径"),
      format: z
    ...
    async (args) => {
      ...
      if (raw === null) return ok({ error: `文件不存在：${args.filePath}` });
      if (raw.trim().length === 0) return ok({ error: "文件内容为空" });

      const result =
        args.format === "gsi-8" ? gsi(raw, false) : args.format === "gsi-16" ? gsi(raw, true) : detect(raw);

      if (!result)
        return ok({ error: "无法识别文件格式，请确认为 GSI 或 DAT 表格格式，或手动指定 format 参数。" });
